#ifndef REINRENDERER_TGAIMAGE_H
#define REINRENDERER_TGAIMAGE_H
...
public:
    ...
    ~TGAImage();
    TGAImage & operator = (const TGAImage &img); // 赋值函数
    int get_width(); // 获取图像宽度
    int get_height(); // 获取图像高度
    int get_bytespp(); // 获取图像每像素字节数
    unsigned char *buffer(); // 获取图像数据指针
    void clear();
};

#endif //REINRENDERER_TGAIMAGE_H
